...
export class ProjectService {
  ...
  getProject$(id: string): Observable<Project | null> {
    const docRef = doc(this.firestore, `projects/${id}`);
    return collectionData(query(collection(this.firestore, 'projects'), where('__name__', '==', id)), { idField: 'id' }).pipe(
      map(docs => docs[0] as Project || null)
    );
  }
   ...
   */
  async createProject(name: string, description: string = '', color: string = '#6366f1'): Promise<DocumentReference> {
    ...
    try {
      ...
      try {
        ...
        await this.updateProject(result.id, { googleTaskListId: taskList.id });
      } catch (err) {
  ...
  async deleteProject(id: string): Promise<void> {
    ...
    try {
      ...
      // Also delete the corresponding Google Tasks list
