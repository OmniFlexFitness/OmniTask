  ...
  template: `
    <div class="image-upload-container">
      ...
      >
        ...
        @if (currentImageUrl()) {
          <img [src]="currentImageUrl()" [alt]="altText()" class="w-full h-full object-cover" />
          <div
          ...
          >
            ...
            @if (uploading()) {
              <div class="flex items-center gap-2">
                <div
                  class="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin border-[#00d2ff]"
                ></div>
        ...
        <!-- Upload Progress Bar -->
        @if (uploading()) {
          <div class="absolute bottom-0 left-0 right-0 h-1 bg-slate-800/80">
            <div
              class="h-full transition-all duration-300 animate-pulse w-[60%] bg-gradient-to-r from-[#e040fb] to-[#00d2ff]"
            ></div>
          </div>
        }
      </div>
